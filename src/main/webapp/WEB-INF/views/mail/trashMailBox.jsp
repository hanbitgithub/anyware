                ...
                <div class="container-fluid">
                  <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0"> 
                      ...
                      <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="#" onclick="emptyMail();">
                            <img src="resources/images/dustbin.png" width='18px' alt="">
                            완전삭제</a>
                      </li>
      
                      <li class="nav-item">
                        <a class="nav-link" href="#" onclick="restoreMail();">
                    ...
                    <form class="d-flex" role="search" action="searchD.em" method="post">
                			...
                			<c:choose>
                				<c:when test="${i.mailStatus.read eq 'Y' }">
			                		<tr style="font-size: 14px;" class="mstatus${i.emNo}"> 
			                			<td width="20"><input type="checkbox" name="check" class="emNo" value="${i.emNo }" data-emtype="${i.mailStatus.emType }"></td>
			                			<td width="25">
			                				<c:choose>
                 ...
                 function restoreMail(){
  						...
  						if( $checked.length < 1){
  							...
  							return false;
  							 ...
  							 if( confirm("선택한 메일을 기존의 메일함으로 복구하시겠습니까?") ){
  								 let checkArr = [];
  								 let typeArr = [];
  								 $(".emNo").each(function(){
       ...
       <!--페이징 영역-->
		<div id="paging-area" align="center">
				...
				<c:otherwise>
					<c:choose>
						<c:when test="${empty keyword and empty filter}">
							<button
								onclick="location.href='trash.em?cpage=${pi.currentPage - 1}'">이전</button>
						</c:when>
						<c:otherwise>
							<button
								onclick="location.href='searchD.em?cpage=${pi.currentPage-1}&keyword=${keyword}&sender=${loginUser.memberId}&receiver=${loginUser.memberId}&filter=${filter }'">이전</button>
						</c:otherwise>
					</c:choose>
				</c:otherwise>
			</c:choose>
			<c:forEach var="p" begin="${pi.startPage }" end="${pi.endPage}">
				<c:choose>
					<c:when test="${empty keyword and empty filter }">
						<button onclick="location.href='trash.em?cpage=${p}'">${p}</button>
					</c:when>
					<c:otherwise>
						<button
							onclick="location.href='searchD.em?cpage=${p}&keyword=${keyword}&sender=${loginUser.memberId}&receiver=${loginUser.memberId}&filter=${filter }'">${p}</button>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:choose>
				...
				<c:otherwise>
					<c:choose>
						<c:when test="${empty keyword and empty filter}">
							<button
								onclick="location.href='trash.em?cpage=${pi.currentPage + 1}'">다음</button>
						</c:when>
						<c:otherwise>
							<button
								onclick="location.href='searchD.em?cpage=${pi.currentPage+1}&keyword=${keyword}&sender=${loginUser.memberId}&receiver=${loginUser.memberId}&filter=${filter }'">다음</button>
						</c:otherwise>
					</c:choose>
				</c:otherwise>
			</c:choose>
