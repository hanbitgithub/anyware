            ...
            <nav class="navbar navbar-expand-lg bg-light">
                <div class="container-fluid">
                  ...
                  <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <form class="d-flex" role="search" action="searchM.em" method="post">
                			...
                			<c:choose>
                				<c:when test="${r.mailStatus.read eq 'Y' }">
			                		<tr style="font-size: 14px;" class="mstatus${r.emNo}"> 
			                			...
			                			<td width="600"class="mail-title">${r.emTitle }<input type="hidden" name="mailNo" value="${r.emNo }"></td>
			                			<td width="50">
		...
		<div id="paging-area" align="center">
			...
			<c:choose>
				...
				<c:otherwise>
					<c:choose>
						...
						<c:otherwise>
							<button
								onclick="location.href='searchM.em?cpage=${pi.currentPage-1}&keyword=${keyword}&receiver=${loginUser.memberId}&filter=${filter }'">이전</button>
						</c:otherwise>
					</c:choose>
				</c:otherwise>
			</c:choose>
			<c:forEach var="p" begin="${pi.startPage }" end="${pi.endPage}">
				<c:choose>
					...
					<c:otherwise>
						<button
							onclick="location.href='searchM.em?cpage=${p}&keyword=${keyword}&receiver=${loginUser.memberId}&filter=${filter }'">${p}</button>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:choose>
				...
				<c:otherwise>
					<c:choose>
						...
						<c:otherwise>
							<button
								onclick="location.href='searchM.em?cpage=${pi.currentPage+1}&keyword=${keyword}&receiver=${loginUser.memberId}&filter=${filter }'">다음</button>
						</c:otherwise>
					</c:choose>
				</c:otherwise>
			</c:choose>
